...
const getInjuryTime = (memo: string) => {
    if (!memo) return 0;
    const match = memo.match(/#mi:\s*(\d+)/);
...
// 🎨 [유틸] 스팟 타입 한글 변환
const SPOT_TYPE_KO: Record<string, string> = {
    FOOD: '식당', CAFE: '카페', LANDMARK: '관광명소',
    HISTORICAL_SITE: '유적지', RELIGIOUS_SITE: '종교시설',
    PARK: '공원', NATURE: '자연', MUSEUM: '박물관/미술관',
    SHOPPING: '쇼핑', ACTIVITY: '액티비티', STATION: '교통/역',
    ACCOMMODATION: '숙소', OTHER: '장소'
};

// 🚗 [유틸] 이동 수단 아이콘 및 이름 매핑
const TRANSPORT_INFO: Record<string, { icon: string, label: string }> = {
    WALK: { icon: '🚶', label: '도보' },
    CAR: { icon: '🚗', label: '차량' },
    BUS: { icon: '🚌', label: '버스' },
...
// ✅ [수정] 구글 Static Map URL 생성 함수 (사이즈 변경)
const getStaticMapUrl = (schedules: DayScheduleResponse[]) => {
    const validPoints = schedules
        .map(s => {
            const temp = decodeTempSpot(s.memo);
            // @ts-ignore
            const lat = Number(s.lat || s.spot?.lat || temp?.lat);
    ...
    // ✅ 1. 지도 사이즈 변경: 600x400 (3:2 비율로 높이 확장)
    const size = "600x400";
    const scale = "2";
    const maptype = "roadmap";

    const markers = validPoints.map((p, idx) => {
        return `markers=color:blue%7Clabel:${idx + 1}%7C${p.lat},${p.lng}`;
    }).join("&");

    const pathCoordinates = validPoints.map(p => `${p.lat},${p.lng}`).join("|");
    const path = `path=color:0x3B82F6ff|weight:5|${pathCoordinates}`;

    return `${baseUrl}?size=${size}&scale=${scale}&maptype=${maptype}&${markers}&${path}&key=${GOOGLE_MAPS_API_KEY}`;
};


// 🎨 [수정] 이미지 저장용 전용 컴포넌트
const DayScheduleExportView = ({ title, memo, schedules }: { title: string, memo: string, schedules: DayScheduleResponse[] }) => {
    const hasMemo = memo && memo.trim().length > 0;
    const staticMapUrl = getStaticMapUrl(schedules);

    return (
        <div id="export-target" className="w-[600px] bg-white flex flex-col font-sans text-gray-800">

            {/* 1. 헤더 */}
            <div className="px-10 pt-16 pb-6 bg-white">
                <span className="inline-block px-3 py-1 bg-blue-600 text-white text-[10px] font-bold rounded-full mb-4 tracking-widest">TRIP PLAN</span>
                <h1 className="text-4xl font-extrabold text-gray-900 leading-tight tracking-tight break-keep mb-8">{title || "여행 일정"}</h1>

                {/* ✅ 2. 메모 패딩 축소 (p-5 -> p-3.5) */}
                {hasMemo && (
                    <div className="bg-gray-50 p-3.5 rounded-2xl border border-gray-100">
                        <span className="block font-bold text-gray-900 text-sm mb-1.5">Note.</span>
                        <p className="text-gray-600 text-sm font-medium whitespace-pre-wrap leading-relaxed">{memo}</p>
                    </div>
                )}
            </div>

            {/* 2. 지도 영역 (Static Map) */}
            {/* ✅ 3. 하단 여백 축소 (pb-8 -> pb-6) */}
            <div className="px-10 pb-6">
                {/* ✅ 4. 지도 높이 확대 (h-64 -> h-[400px]) */}
                <div className="w-full h-[400px] bg-gray-100 rounded-2xl border border-gray-200 flex flex-col items-center justify-center text-gray-400 gap-2 relative overflow-hidden shadow-sm">
                    {staticMapUrl ? (
                        <img
                            src={staticMapUrl}
                            alt="Route Map"
            ...
            {/* 3. 일정 리스트 (이전 코드 유지) */}
            <div className="px-10 py-6 flex flex-col gap-0 relative bg-white min-h-[200px]">
                {schedules.map((item, index) => {
                    const isPlace = item.spotId !== 0 || !!decodeTempSpot(item.memo);
                    const cleanMemo = cleanMemoTags(item.memo);
                    const hasItemMemo = cleanMemo && cleanMemo.trim().length > 0;
                    const endTime = item.duration ? addTime(item.startTime, item.duration) : "";

                    const transport = TRANSPORT_INFO[item.transportation] || TRANSPORT_INFO['CAR'];
                    // @ts-ignore
                    const injuryTime = getInjuryTime(item.movingMemo || item.memo || "");
                    const netMovingDuration = Math.max(0, item.movingDuration - injuryTime);

                    return (
                        <div key={index} className="flex relative pb-14 last:pb-0 group">
                            {index !== schedules.length - 1 && (
                                <div className="absolute left-[89px] top-10 bottom-[-20px] w-[2px] bg-gray-100 z-0"></div>
                            )}

                            {index > 0 && item.movingDuration > 0 && (
                                <div className="absolute left-[89px] -translate-x-1/2 -top-[34px] z-20">
                                    <div className="flex items-center gap-1 bg-white border border-gray-200 px-2 py-0.5 rounded-full shadow-[0_2px_4px_rgba(0,0,0,0.05)]">
                      <span className="text-[9px] text-gray-500 flex items-center gap-0.5">
                        {transport.icon} <span className="tracking-tight">{transport.label}</span>
                      </span>
                                        <div className="w-[1px] h-2 bg-gray-200 mx-0.5"></div>
                                        <span className="text-[9px] font-bold text-gray-700 whitespace-nowrap font-mono">{netMovingDuration}분</span>
                                    </div>
                                </div>
                            )}

                            <div className="w-[70px] pt-[26px] text-right pr-5 flex flex-col items-end shrink-0">
                                <span className="text-sm font-bold text-gray-900 font-mono tracking-tight">{item.startTime}</span>
                                {endTime && <span className="text-[10px] font-medium text-gray-400 font-mono mt-0.5">{endTime}</span>}
                            </div>

                            <div className="relative z-10 w-10 flex justify-center pt-[28px] shrink-0">
                                {isPlace ? (
                                    <div className="w-3.5 h-3.5 rounded-full bg-blue-600 ring-4 ring-white shadow-sm"></div>
                                ) : (
                            ...
                            <div className="flex-1 pl-3 pt-1">
                                <div className="flex flex-col items-start">
                                    {item.spotType !== 'OTHER' && (
                                        <span className="mb-1 text-[10px] font-bold text-blue-500 bg-blue-50/50 px-1.5 py-0.5 rounded uppercase tracking-wider">
                      {SPOT_TYPE_KO[item.spotType] || item.spotType}
                    </span>
                                    )}
                                    <h3 className={`text-xl font-bold leading-snug ${isPlace ? 'text-gray-900' : 'text-gray-400'}`}>
                                        {item.spotName || "이동 / 자유시간"}
                                    </h3>
                                    {hasItemMemo && (
                                        <div className="mt-2 text-xs text-gray-500 leading-relaxed border-l-2 border-gray-200 pl-3">
                                            {cleanMemo}
                                        </div>
                                    )}
                                </div>
            ...
            {/* 하단 푸터 */}
            <div className="px-10 py-8 mt-4 border-t border-gray-100 flex justify-between items-end">
                <span className="text-[10px] font-bold text-gray-300 tracking-widest uppercase">YOUR UNIQUE MEMORY EXPEDITION • YUME</span>
                <span className="text-[10px] font-medium text-gray-300">Plan your unique trip</span>
            </div>
        </div>
    );
};
...
function DayDetailContent() {
    ...
    // ✅ [수정] 이미지 저장 핸들러 (숨겨진 Export 뷰를 캡처)
    const handleSaveImage = async () => {
        const element = document.getElementById("export-target");
        if (!element) return alert("저장할 영역을 찾을 수 없습니다.");
        ...
        try {
            ...
            link.download = `${day?.dayName || "일정"}.png`;
            link.href = dataUrl;
            link.click();
        } catch (err: any) {
        ...
        <>
            ...
            <div className="flex w-full h-full relative">
                ...
                <div className={`flex flex-col w-full h-full bg-white md:w-1/2 relative z-10 transition-transform duration-300 ${mobileViewMode === 'MAP' ? 'translate-x-full md:translate-x-0' : 'translate-x-0'}`}>
                    ...
                    <div className="px-5 py-4 border-b border-gray-100 bg-white/95 backdrop-blur z-30 flex-shrink-0 flex flex-col gap-3">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2 shrink-0">
                                <button onClick={handleSaveImage} className="p-2 text-gray-500 bg-gray-100 rounded-lg hover:bg-gray-200" title="이미지로 저장">📸</button>
                                {isDirty && <button onClick={handleReset} className="p-2 text-gray-400 bg-gray-100 rounded-lg hover:bg-gray-200">↺</button>}
                                <button onClick={handleSaveAll} disabled={!isDirty} className={`px-3 py-2 rounded-lg font-bold text-sm transition shadow-sm ${isDirty ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-gray-100 text-gray-400 cursor-default'}`}>{isDirty ? '저장' : '완료'}</button>
                            </div>
                        </div>
                    ...
                    {/* ✅ 이미지 캡처 대상 영역 ID 부여 */}
                    <div id="day-schedule-container" className="flex-1 overflow-y-auto p-4 pb-24 bg-white scrollbar-hide">
