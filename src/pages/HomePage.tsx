      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-10 px-4 md:px-0">
        ...
        <section className="pt-4">
          ...
          <p className="text-gray-500 text-sm mt-1">
            {upcomingPlan ? '여행 일정과 현재 상황을 확인하세요.' : '새로운 여행을 떠나보세요!'}
          </p>
        </section>
        ...
        {!upcomingPlan ? (
            <div className="space-y-6 animate-fade-in-up">
              <div className="bg-white rounded-2xl p-8 border border-gray-200 text-center shadow-sm">
                ...
                <Link to="/plans/create" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl transition">
                  <span>+</span> 새 여행 계획하기
                </Link>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
              <div className="relative z-10">
                ...
                <div className="mt-4 text-right">
                  <Link to={`/plans/${upcomingPlan.id}`} className="text-sm font-bold hover:text-blue-100 transition">상세 일정 보기 &rarr;</Link>
                </div>
              </div>
            </div>
        ) : (
            <div className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
              <div className="bg-blue-50 p-5 border-b border-blue-100 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{upcomingPlan.planName}</p>
                </div>
                <Link to={`/plans/${upcomingPlan.id}`} className="text-sm text-blue-600 font-bold hover:underline whitespace-nowrap ml-2">전체 일정</Link>
              </div>
                    ...
                    <APIProvider apiKey={GOOGLE_MAPS_API_KEY} libraries={['maps', 'marker']}>
                      ...
                      >
                        ...
                        {todaySchedules.map((schedule, index) => {
                          ...
                          // 핀 색상 결정
                          let markerColor = "#3B82F6"; // 기본 파랑
                          let isActive = false;

                          if (schedule.id === activeScheduleId) {
                            markerColor = "#EF4444"; // 🔴 체류 중 (빨강)
                            isActive = true;
                          } else if (index === activeMovingIndex) {
                            markerColor = "#F97316"; // 🟠 이동 도착지 (주황)
                            isActive = true;
                          } else if (index === activeMovingIndex - 1) {
                            markerColor = "#10B981"; // 🟢 이동 출발지 (초록)
                            isActive = true;
                          }
              ...
              <div className="p-5">
                ...
                {scheduleLoading ? (
                    <div className="py-10 text-center text-gray-400">일정을 불러오는 중...</div>
                ) : todaySchedules.length > 0 ? (
                            ...
                            <div key={item.id}>
                              ...
                              {index > 0 && item.movingDuration > 0 && (
                                  <div className="flex group h-12">
                                    {/* 1. 시간 영역 (이동 시작 시간 표시) */}
                                    <div className="w-14 shrink-0 text-right pt-3 pr-1">
                                    ...
                                    <div className="w-10 shrink-0 flex flex-col items-center relative">
                                      <div className="w-0.5 bg-gray-200 h-full absolute top-0 bottom-0"></div>
                                      <div className={`relative z-10 w-6 h-6 rounded-full flex items-center justify-center text-xs shrink-0 border shadow-sm mt-3 transition-colors
                                            ${isMovingActive ? 'bg-blue-600 border-blue-600 text-white animate-pulse' : 'bg-white border-blue-200 text-blue-500'}`}>
                                        {getTransIcon(item.transportation)}
                                      </div>
                                    </div>
                                    <div className="flex-1 min-w-0 py-1 pl-2">
                                      <div className={`flex items-center gap-2 rounded-lg px-3 py-1.5 border transition-all
                                            ${isMovingActive
                                          ? 'bg-blue-100 border-blue-300 ring-1 ring-blue-300 shadow-sm'
                                          : 'bg-blue-50/50 border-blue-50/50'}`}>
                                            <span className={`text-xs font-bold truncate ${isMovingActive ? 'text-blue-800' : 'text-blue-600'}`}>
                                                {cleanMemoTags(item.movingMemo) || `${getTransLabel(item.transportation)} 이동`}
                                            </span>
                                        <span className={`text-[10px] font-bold shrink-0 ${isMovingActive ? 'text-blue-700' : 'text-gray-500'}`}>
                                                {pureMovingDuration}분
                                          {moveInjury > 0 && <span className="text-orange-400 ml-1">(+{moveInjury})</span>}
                                            </span>

                                        {/* ✅ [수정] 이동 중 뱃지도 NOW로 통일 */}
                                        {isMovingActive && (
                                            <span className="text-[10px] bg-blue-600 text-white px-1.5 rounded ml-auto font-bold animate-pulse">
                                                    NOW
                                                </span>
                                        )}
                                      </div>
                              ...
                              <div className="flex group min-h-[4rem]">
                                <div className="w-14 shrink-0 text-right pt-2 pr-1">
                                  <span className={`text-sm font-bold block font-mono ${isActive ? 'text-orange-600' : 'text-gray-900'}`}>
                                ...
                                <div className="w-10 shrink-0 flex flex-col items-center relative">
                                  {index !== todaySchedules.length - 1 && (
                                      <div className="w-0.5 bg-gray-200 absolute top-0 bottom-0 h-full"></div>
                                  )}
                                  <div className={`relative z-10 w-9 h-9 rounded-full flex items-center justify-center text-lg shrink-0 border shadow-sm transition-transform
                                        ${isActive ? 'bg-orange-500 border-orange-600 text-white scale-110' : 'bg-white border-gray-200 group-hover:scale-110'}`}>
                                    {typeInfo.icon}
                                  </div>
                                </div>

                                <div className="flex-1 min-w-0 pb-6 pl-2">
                                  <div className={`rounded-xl p-3 border shadow-sm transition relative overflow-hidden
                                        ${isActive ? 'bg-orange-50 border-orange-200 ring-1 ring-orange-300' : 'bg-white border-gray-200 hover:border-orange-200'}`}>
                                    ...
                                    <div className="flex justify-between items-start gap-2">
                                      <h4 className={`text-sm font-bold truncate min-w-0 flex-1 ${item.spotName || tempSpot?.name ? 'text-gray-900' : 'text-gray-400 italic'}`}>
                                        {displayName}
                                      </h4>
                                      <span className="text-[10px] text-gray-500 bg-gray-50 px-2 py-1 rounded-md border border-gray-100 shrink-0 whitespace-nowrap">
                                                체류 {item.duration}분
                                            </span>
                                    </div>

                                    {displayMemo && <p className="text-xs text-gray-500 mt-2 line-clamp-2 bg-white/50 p-2 rounded-lg border border-gray-100">{displayMemo}</p>}
                                  </div>
                                </div>
                              </div>
                            </div>
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                      ...
                      <Link to={`/plans/${upcomingPlan.id}`} className="text-blue-600 text-xs font-bold mt-2 inline-block">+ 일정 추가하기</Link>
                    </div>
                )}
              </div>
            </div>
        ...
        {upcomingPlan && (
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
              <Link to="/map" className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition">
                <div className="text-xl">🗺️</div><span className="text-sm font-bold text-gray-700">장소 탐색</span>
              </Link>
              <Link to="/spots" className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition">
                <div className="text-xl">📍</div><span className="text-sm font-bold text-gray-700">내 장소</span>
              </Link>
            </div>
        )}
      </div>
